...
namespace Task5
{
    public class DataBase
    {
        ...
        static public int JSONserializer(string? filePath, List<Student> students)
        {
            string ans = "";
            ans += "[\n";
            ...
            {
                ans += "\t{\n";
                ans += $"\t\t\"fullName\": \"{students[i].fullName}\",\n";
                ans += "\t\t\"birthdayDate\": {\n";
                ans += $"\t\t\t\"day\": {students[i].birthdayDate.day},\n";
                ans += $"\t\t\t\"month\": {students[i].birthdayDate.month},\n";
                ans += $"\t\t\t\"year\": {students[i].birthdayDate.year}\n";
                ans += "\t\t},\n";
                ans += $"\t\t\"institute\": \"{students[i].institute}\",\n";
                ans += $"\t\t\"group\": \"{students[i].group}\",\n";
                ans += $"\t\t\"course\": {students[i].course},\n";
                ans += $"\t\t\"avgMark\": {string.Join(".", students[i].avgMark.ToString().Split(","))},\n";
                ans += $"\t\t\"educationForm\": \"{students[i].educationForm}\",\n";
                ans += $"\t\t\"educationLevel\": \"{students[i].educationLevel}\",\n";
                ans += $"\t\t\"numDolg\": {students[i].numDolg}\n";
                ans += "\t},\n";
            }
            ans += "\t{\n";
            ans += $"\t\t\"fullName\": \"{students[students.Count - 1].fullName}\",\n";
            ans += "\t\t\"birthdayDate\": {\n";
            ans += $"\t\t\t\"day\": {students[students.Count - 1].birthdayDate.day},\n";
            ans += $"\t\t\t\"month\": {students[students.Count - 1].birthdayDate.month},\n";
            ans += $"\t\t\t\"year\": {students[students.Count - 1].birthdayDate.year}\n";
            ans += "\t\t},\n";
            ans += $"\t\t\"institute\": \"{students[students.Count - 1].institute}\",\n";
            ans += $"\t\t\"group\": \"{students[students.Count - 1].group}\",\n";
            ans += $"\t\t\"course\": {students[students.Count - 1].course},\n";
            ans += $"\t\t\"avgMark\": {string.Join(".", students[students.Count - 1].avgMark.ToString().Split(","))},\n";
            ans += $"\t\t\"educationForm\": \"{students[students.Count - 1].educationForm}\",\n";
            ans += $"\t\t\"educationLevel\": \"{students[students.Count - 1].educationLevel}\",\n";
            ans += $"\t\t\"numDolg\": {students[students.Count - 1].numDolg}\n";
            ans += "\t}\n";
            ans += "]\n";
            StreamWriter? wstream = WriteFile(filePath);
            if (wstream == null)
            {
                return -1;
            }
        ...
        static public string FindValue(string line)
        {
            string result = "";
            ...
            bool isDoublescore = false, isInt = false;
            int l = 0, r = 0, temp = 0;
            for (int i = 0; i < result.Length; i++)
            {
                if(!isDoublescore)
                {
                    if (!isInt)
                    {
                        ...
                        if (int.TryParse(result[i].ToString(), out temp))
                        {
                            isInt = true;
                            l = i;
                    ...
                    {
                        if (int.TryParse(result[i].ToString(), out temp) && i == result.Length - 1)
                        {
                            r = i + 1;
                            break;
                        }
            ...
            return result.Substring(l, r - l);
        }

        static public List<Student> JSONdeserializator(string dbpath)
        {
            List<Student> students = new List<Student>();
            StreamReader? wstream = DataBase.InputFile(dbpath);
            if (wstream == null)
            {
                return null;
            }
            ...
            {
                student.idStudent = acc;
                if (OnString(lines[i], "\"fullName\""))
                {
                    //TODO: находишь :, потом проверка, нсть ли дальше ' "" ', если есть, то два указателя на них, если нет, то на цифры, код пусть руинится при неправильном заполнении JSONa.
                    student.fullName = FindValue(lines[i]);
                }
                else if (OnString(lines[i], "\"day\""))
                {
                    student.birthdayDate.day = int.Parse(FindValue(lines[i]));
                }
                else if (OnString(lines[i], "\"month\""))
                {
                    student.birthdayDate.month = int.Parse(FindValue(lines[i]));
                }
                else if (OnString(lines[i], "\"year\""))
                {
                    student.birthdayDate.year = int.Parse(FindValue(lines[i]));
                }
                else if (OnString(lines[i], "\"institute\""))
                {
                    student.institute = FindValue(lines[i]);
                }
                else if (OnString(lines[i], "\"group\""))
                {
                    student.group = FindValue(lines[i]);
                }
                else if (OnString(lines[i], "\"course\""))
                {
                    student.course = int.Parse(FindValue(lines[i]));
                }
                else if (OnString(lines[i], "\"avgMark\""))
                {
                    student.avgMark = double.Parse(FindValue(lines[i]));
                }
                else if (OnString(lines[i], "\"educationForm\""))
                {
                    student.educationForm = FindValue(lines[i]);
                }
                else if (OnString(lines[i], "\"educationLevel\""))
                {
                    student.educationLevel = FindValue(lines[i]);
                }
                else if (OnString(lines[i], "\"numDolg\""))
                {
                    student.numDolg = int.Parse(FindValue(lines[i]));
                }
            ...
            while (true)
            {
                ...
                switch (input)
                {
                    ...
                    case "2":
                        ...
                        Console.Write("Для отмены введите 'back': ");
                        string? studentStr;
                        try
                        {
                    ...
                    case "10":
                        ...
                        while (true) 
                        {
                            ...
                            if (fileName == null || fileName == "" || DataBase.OnString(fileName, "."))
                            {
                                Console.Write("Невозможно создать файл с таким именем. Повторите ввод: ");
                                continue;
                            }
                            break;
                        }
                        string? filePath = null;
                        Console.Write("Введите путь, по которому будет расположен файл: ");
                        while (true)
                        {
                            ...
                            {
                                Console.WriteLine("Файл с данным именем существует. Вы хотите его заменить? (Y/n)? ");
                                string? choose = Console.ReadLine();
                                if (choose == "Y" || choose == "y")
                                {
                                    if (DataBase.JSONserializer(filePath, studentList) != 0)
                                    {
                                        Console.WriteLine("Невозможно сериализовать данные.");
                                    }
                                    else
                                    {
                                        Console.WriteLine("Сериализация завершена! Файл сохранен");
                                    }
                                    break;
                                }
                                ...
                                {
                                    Console.Write("Некорректный ответ. Повторите ввод: ");
                                    continue;
                                }
                            }
                            else
                    ...
                    case "11":
                        ...
                        while (true)
                        {
                            filePathJSON = Console.ReadLine();
                            if (File.Exists(filePathJSON))
                            {
                                List<DataBase.Student> temp = new List<DataBase.Student>();
                                try
                                {
                                    temp = DataBase.JSONdeserializator(filePathJSON);
                                }
                                catch
                                {
                                    Console.WriteLine("Ошибка при чтении файла. Проверьте корректность данных в файле");
                                }
                                if (temp != new List<DataBase.Student>())
                                {
                                    studentList = temp;
                                    Console.WriteLine("База данных составлена!");
                                    break;
                                }
                                else
                                {
                                    Console.WriteLine("Десериализация невозможна.");
                                }

                            }
                            else
                            {
                                Console.Write("Файл не найден. Повторите ввод, не забыв указать имя файла с расширением в конце пути: ");
                            }
                        }
                        continue;
                    ...
                    case "help":
                        ...
                        Console.WriteLine("10 - сериализация базы данных в формат JSON");
                        Console.WriteLine("11 - десериализация из формата JSON");
